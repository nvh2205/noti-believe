...
if git pull; then
    ...
    echo -e "${YELLOW}You might need to run: sudo git config --global --add safe.directory $(pwd)${NC}"
    exit 1
fi
...
echo -e "${YELLOW}You can check the logs with: sudo docker-compose logs -f${NC}" 